function [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy,TY,pred_label,actual_label] = elm_kernel(train_data, test_data, Elm_Type, Regularization_coefficient, Kernel_type, Kernel_para)
...
OutputWeight=((Omega_train+speye(n)/C)\(T')); %speye（n）生成稀疏单位阵
TrainingTime=toc;
...
%%%%%%%%%% Calculate training & testing classification accuracy
    ...
    TrainingAccuracy=(1-MissClassificationRate_Training/size(T,2))*100;  
    for i = 1 : size(TV.T, 2)
        [x, label_index_expected]=max(TV.T(:,i));
        [x, label_index_actual]=max(TY(:,i));
        pred_label = [pred_label, label_index_actual];
        actual_label=[actual_label,label_index_expected];
        if label_index_actual~=label_index_expected
